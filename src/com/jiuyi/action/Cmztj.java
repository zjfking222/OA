 ...
 *
 */
public class Cmztj  extends BaseAction{
			...
			String wwcfhd = "";//未完成发货单
			 try{
				...
				while(rs.next()){
					...
					String POSNR = rs.getString("POSNR");//行项目数量
					sql="select * from "+tablename+"_dt1  where mainid='"+wwcfhd+"' and POSNR='"+POSNR+"'";
					rs1.execute(sql);
					rs1.next();
					double ZZWFHL = JiuyiUtil.getDoubleFromQfw(rs1.getString("ZZWFHL"));//未发货量
					if(ZZBCFHL>ZZWFHL){
						error = 1;
						message = "商品"+MAKTX+"本次发货数量"+ZZBCFHL+"超过未发货量"+ZZWFHL;
						break;
					}
				}
				if(error == 1){
