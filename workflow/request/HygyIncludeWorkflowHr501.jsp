...
<%
    ...
    if(formid!=-1){
        ...
        String leix = map.get("leix").toString();//请假类型
...
<script type="text/javascript">
    ...
    function getShis(){
        ...
        if(h_leix==''){
            h_leix = jQuery("#disfield"+leix+" option:selected").text();
        }
        if(h_yuangbh=="" || h_ksrq=="" || h_kssj==":" || h_jsrq=="" || h_jssj==":"|| h_leix==""){
            return;
        }else{
            jQuery.ajax({
                ...
                async:false,
                type:"post",
                data:{"operation":"time","h_yuangbh":h_yuangbh,"h_ksrq":h_ksrq,"h_kssj":h_kssj,"h_jsrq":h_jsrq,"h_jssj":h_jssj,"h_leix":h_leix},
                success:function(data){
                    eval("var obj="+data);
                    setFMVal(ss,obj.shis);//时数
                    setFMVal(nianxjde,obj.nianxjde);//年休假定额
                    var ssValue=jQuery("#field"+ss).val();
