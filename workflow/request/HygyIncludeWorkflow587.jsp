...
<%
//供应商停用流程  读取BPC模版接口js 
new BaseBean().writeLog("供应商停用数据流程  读取BPC模版接口js ");
...
String LIFNR =  map.get("lifnr");//供应商编码
String EKORG =  map.get("ekorg");//采购组织
String BUKRS =  map.get("bukrs");//公司代码*
String LFB1_LOEVM =  map.get("lfb1_loevm");//基于公司代码的删除标记
String LFM1_LOEVM =  map.get("lfm1_loevm");//基于采购组织的删除标记


HashMap<String, String> map2 = CommonUtil.getFieldId(formid,"1");//明细1
String BANKA = map2.get("banka");//银行名称
String BANKN = map2.get("bankn");//银行名称
String BKREF = map2.get("bkref");//参考明细
...
var EKORG = '<%=EKORG%>';//采购组织
var BUKRS = '<%=BUKRS%>';//公司代码*
var LFB1_LOEVM = '<%=LFB1_LOEVM%>';//基于公司代码的删除标记
var LFM1_LOEVM = '<%=LFM1_LOEVM%>';//基于采购组织的删除标记
/*****************明细1********************/
var BANKA = '<%=BANKA%>';//银行名称
var BANKN = '<%=BANKN%>';//银行帐户号码
var BKREF = '<%=BKREF%>';//参考明细
...
function bindchange(){
	jQuery("#field"+LIFNR).bindPropertyChange(function(){
		var LIFNR1 = jQuery("#field"+LIFNR).val();
		var EKORG1 = jQuery("#field"+EKORG).val();
		var BUKRS1 = jQuery("#field"+BUKRS).val();
		setDetailData(LIFNR1,EKORG1,BUKRS1);
	});
}

//添加明细行数据
function setDetailData(LIFNR1,EKORG1,BUKRS1){
	if(LIFNR1==''||EKORG1==''){
		return;
	}else{
		jQuery.ajax({
			url:"/workflow/request/GetSAPDataAjax587.jsp",
			type:"post",
			data:{"LIFNR1":LIFNR1,"EKORG1":EKORG1,"BUKRS1":BUKRS1},
			async: true,
...
function setDt1Data(dt1Data){
	...
	for(var i=0;i<dt1Data.length;i++){
		lineData=dt1Data[i];
		addRow0(0);
		setFMVal(BANKA+"_"+i, lineData.BANKA);//银行名称
		setFMVal(BANKN+"_"+i, lineData.BANKN);//银行帐户号码
		setFMVal(BKREF+"_"+i, lineData.BKREF);//参考明细
	}
} 
...
function setDt2Data(dt2Data){
	var lineData;
	lineData=dt2Data[0];
	if(lineData.LFB1_LOEVM =="X"){
		jQuery("#field"+LFB1_LOEVM).nextAll().addClass("jNiceChecked");//冻结标示
		}else if(lineData.LFB1_LOEVM ==""){
			jQuery("#field"+LFB1_LOEVM).nextAll().removeClass("jNiceChecked");//冻结标示
			}
	if(lineData.LFM1_LOEVM =="X"){
		jQuery("#field"+LFM1_LOEVM).nextAll().addClass("jNiceChecked");//冻结标示
		}else if(lineData.LFM1_LOEVM ==""){
			jQuery("#field"+LFM1_LOEVM).nextAll().removeClass("jNiceChecked");//冻结标示
			}
} 
